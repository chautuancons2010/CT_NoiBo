# Soft Colorful Enterprise UI

The reference screenshots are the highest priority visual specification. The product uses a warm gray and lavender neutral canvas, large rounded surfaces, full pastel KPI cards, soft pill navigation and restrained borders. Do not reinterpret the theme as a blue and white corporate admin UI.

## Architecture

`src/app/theme.css` is the canonical source for visual decisions. It loads after `globals.css` from the root layout. Existing module class names continue to work through compatibility selectors in the theme. New presentation code should use shared components from `src/components/shared` and semantic tokens from the theme. `src/app/globals.css` still contains historical layout and module rules; the migration is in progress. Do not add new visual constants there.
...
Module presentation is centralized in `src/config/moduleIconRegistry.ts`. Change a module icon, pastel surface or accent there. `ModuleLauncher` filters `applicationRegistry` through the current user's permissions; `ModuleIconCard` owns the shared hover, focus and pressed behavior. Feature pages must not define module icon colors themselves.
...
| Text | `--text-primary`, `--text-secondary`, `--text-muted`, `--text-disabled` | Edit root tokens; keep text contrast on pastel backgrounds |
| Accent | `--color-primary` and its related tokens from appearance settings; aliases `--primary`, `--primary-hover`, `--primary-soft` | Change the configured primary color or the root layout brand token mapping |
| Pastels | `--pastel-cream`, `--pastel-yellow`, `--pastel-lime`, `--pastel-lavender`, `--pastel-violet`, `--pastel-mint`, `--pastel-cyan`, `--pastel-blue`, `--pastel-peach`, `--pastel-rose`, plus compatible color scales | Edit one palette token; do not put literal hex in a page |
...
| Radius | `--radius-control`, `--radius-button`, `--radius-nav`, `--radius-card`, `--radius-card-large`, `--radius-panel`, `--radius-pill` | Change the semantic token; standard cards currently use 22px and large dashboard cards use 26px |
...
- `MetricCard`, `ChartCard`, `AnalyticsCard`, `ProgressCard`, `ActivityCard`, `TaskCard`: dashboard surfaces. KPI cards use full pastel backgrounds; chart and analytics cards use the shared large radius.
...
- `FormSection`, `Input`, `Select`, `Textarea`, `Checkbox`, `Switch`, `StickyActionBar`: forms.
...
Buttons rise 1px on hover, interactive cards rise 3px with a 1.005 scale, rows tint without movement, navigation links move 2px horizontally and icon buttons scale to 1.02. Floating surfaces fade and move 3px using `--shadow-floating`. Focus uses the configured primary ring. Icon buttons require an accessible `label`. Avoid using pastel color alone to explain state. `prefers-reduced-motion` disables these transitions and animations.
...
Completed in this pass: warm pastel theme tokens and compatibility mapping, shell quick create, core button/card variant API, dashboard/list/form/detail/report templates, employee list/create/detail composition, shared dashboard card primitives, full pastel KPI geometry, custom line/bar/circular analytics, dashboard recent activity table and dev-only `/__ui` preview. The personal Dashboard now includes the current user's greeting, permission aware module launcher, contextual date, real KPI/chart data, schedule/deadline and notification rail, operational module widgets, recent data, tasks, and persisted personal notes and todos. Shared CSS applies to the existing module markup across the authenticated app.
...
Validation on the current working tree: `npm run typecheck`, `npm run lint`, `npm test -- --run` (57 files, 192 tests), `npm run build`, `npm run ui:audit` (179 authenticated routes), and `npm run ui:qa` passed. Viewport QA verifies login at 320–2560px, zoom, font loading, protected route redirects, plus the shared playground at 390px and 1440px. It asserts the warm background, 22px card radius, 44px control height, KPI cards and DataTable, and saves screenshots in `docs/ui-evidence`. A production server check returned HTTP 404 for `/ui-preview`. Run the development seed against a disposable Supabase instance before manual authenticated workflow QA. Build logged `system_settings.read_failed` while generating pages because the local settings data source was unavailable, but the command finished successfully.
...
8. Module icon, pastel, or accent: edit the module entry in `moduleIconRegistry`.
