...
export async function publishNotificationEvent(input:{eventKey:string;aggregateType:string;aggregateId:string;actorAccountId?:string;correlationId?:string;idempotencyKey:string;recipients:string[];values:Record<string,string>;deepLink?:string;priority?:AppNotification["priority"]},client?:SupabaseClient){
  ...
  const{data:template}=await database.from("notification_templates").select("*").eq("event_key",input.eventKey).eq("active",true).maybeSingle();if(!template)return{eventId:event.id,created:0};const allowed=template.allowed_placeholders as string[];const errors=[...validateTemplate(template.title_template,allowed),...validateTemplate(template.message_template,allowed)];if(errors.length)throw new AppError("VALIDATION_ERROR",errors[0]);const category=eventCategory(input.eventKey);let created=0;
  for(const recipient of [...new Set(input.recipients)]){const{data:preference}=await database.from("notification_preferences").select("in_app_enabled").eq("account_id",recipient).eq("category",category).maybeSingle();if(!template.mandatory&&preference?.in_app_enabled===false)continue;const deliveryKey=`${input.idempotencyKey}:${recipient}:${template.id}:in_app`;const{error}=await database.from("notifications").upsert({recipient_account_id:recipient,event_id:event.id,event_key:input.eventKey,type:input.eventKey.toUpperCase().replaceAll(".","_"),title:renderTemplate(template.title_template,input.values),message:renderTemplate(template.message_template,input.values),entity_type:input.aggregateType,entity_id:input.aggregateId,deep_link:input.deepLink??null,priority:input.priority??"normal",template_version:template.version,delivery_key:deliveryKey},{onConflict:"delivery_key",ignoreDuplicates:true});if(!error)created++;}
  await database.from("domain_events").update({processed_at:new Date().toISOString()}).eq("id",event.id);return{eventId:event.id,created};
}
