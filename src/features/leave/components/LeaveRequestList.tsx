...
export function LeaveRequestList({scope="self"}:{scope?:"self"|"all"|"approval"}){
  ...
  useEffect(()=>{fetch(`/api/v1/leave-requests?scope=${scope}`).then(async response=>{const body=await response.json();if(!response.ok)throw new Error(body.error?.message);setItems(body.data);}).catch(value=>setError(value.message)).finally(()=>setLoading(false));},[scope]);
