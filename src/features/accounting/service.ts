...
export async function createPayroll(user: AuthenticatedUser, input: z.infer<typeof payrollCreateSchema>) {
  if (!can(user.permissions, "payroll.create")) throw new AppError("PERMISSION_DENIED");
  const client = db();
  const actor = await identity(client, user);
  const { data: timesheet, error: timesheetError } = await client.from("timesheet_periods").select("id,status,start_date").eq("id", input.timesheetPeriodId).maybeSingle();
  if (timesheetError) throw new AppError("SERVER_ERROR", "Không thể đọc bảng công.");
  if (!timesheet || timesheet.status !== "locked") throw new AppError("VALIDATION_ERROR", "Bảng công phải được chốt trước khi lập bảng lương.");

  const month = `${String(timesheet.start_date).slice(0, 7)}-01`;
  const [{ data: existingForTimesheet, error: existingTimesheetError }, { data: existingForMonth, error: existingMonthError }] = await Promise.all([
    client.from("payroll_periods").select("id").eq("timesheet_period_id", input.timesheetPeriodId).maybeSingle(),
    client.from("payroll_periods").select("id,timesheet_period_id").eq("period_month", month).maybeSingle()
  ]);
  if (existingTimesheetError || existingMonthError) throw new AppError("SERVER_ERROR", "Không thể kiểm tra bảng lương.");
  if (existingForTimesheet?.id) return getPayroll(user, String(existingForTimesheet.id));
  if (existingForMonth?.id) throw new AppError("CONFLICT", "Tháng này đã có bảng lương liên kết với một bảng công khác.");

  const { data, error } = await client.from("payroll_periods").insert({ period_month: month, timesheet_period_id: input.timesheetPeriodId, note: input.note ?? null, created_by: actor.accountId }).select("id").single();
  if (error || !data) throw new AppError("SERVER_ERROR", "Không thể tạo bảng lương.");
  await recordAuditLog({ actorId: actor.accountId, action: "payroll.created", entityType: "payroll_period", entityId: String(data.id), after: input });
  return getPayroll(user, String(data.id));
}
...
async function calculate(client: SupabaseClient, period: PayrollDetail, actorId: string) {
  if (["locked", "published"].includes(period.status)) throw new AppError("CONFLICT", "Bảng lương đã khóa.");
  if (!period.timesheetPeriodId) throw new AppError("VALIDATION_ERROR", "Bảng lương chưa liên kết bảng công.");
  const end = new Date(`${period.periodMonth}T00:00:00Z`); end.setUTCMonth(end.getUTCMonth() + 1); end.setUTCDate(0);
  const endDate = end.toISOString().slice(0, 10);
  const [{ data: summaries, error: summaryError }, { data: salaries, error: salaryError }] = await Promise.all([
    client.from("timesheet_period_summaries").select("employee_id,scheduled_workdays,actual_workdays").eq("period_id", period.timesheetPeriodId),
    client.from("employee_salary_history").select("*").lte("effective_date", endDate).order("effective_date", { ascending: false })
  ]);
  if (summaryError || salaryError) throw new AppError("SERVER_ERROR", "Không thể đọc dữ liệu để tính bảng lương.");
  const roster = summaries ?? [];
  if (!roster.length) throw new AppError("VALIDATION_ERROR", "Bảng công chưa có dữ liệu tổng hợp để tính lương.");
  const salaryByEmployee = new Map<string, Row>();
  for (const item of salaries ?? []) if (!salaryByEmployee.has(String(item.employee_id))) salaryByEmployee.set(String(item.employee_id), item as Row);
  const missingSalary = roster.filter((item) => !salaryByEmployee.has(String(item.employee_id)));
  if (missingSalary.length) throw new AppError("VALIDATION_ERROR", `Còn ${missingSalary.length} nhân viên chưa có mức lương hiệu lực. Hãy cập nhật Hồ sơ lương trước khi tính.`);
  const rows = roster.map((summary) => {
    const current = salaryByEmployee.get(String(summary.employee_id))!;
    const scheduled = Number(summary.scheduled_workdays ?? 0), actual = Number(summary.actual_workdays ?? 0), ratio = scheduled > 0 ? Math.min(1, actual / scheduled) : 1;
    return { payroll_period_id: period.id, employee_id: summary.employee_id, work_days: actual, base_salary: Math.round(Number(current.base_salary) * ratio), allowance: Number(current.allowance), bonus: Number(current.bonus), deduction: Number(current.deduction), calculation_snapshot: { salaryHistoryId: current.id, effectiveDate: current.effective_date, scheduledWorkdays: scheduled, actualWorkdays: actual, ratio }, updated_at: new Date().toISOString() };
  });
  const { error } = await client.from("payroll_lines").upsert(rows, { onConflict: "payroll_period_id,employee_id" });
  if (error) throw new AppError("SERVER_ERROR", "Không thể tính bảng lương.");
  const { data: changed, error: updateError } = await client.from("payroll_periods").update({ status: "calculated", calculated_at: new Date().toISOString(), calculated_by: actorId, row_version: period.rowVersion + 1, updated_at: new Date().toISOString() }).eq("id", period.id).eq("row_version", period.rowVersion).select("id").maybeSingle();
  if (updateError || !changed) throw new AppError("CONFLICT", "Bảng lương vừa được cập nhật.");
}
