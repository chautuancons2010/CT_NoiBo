...
export function ProjectList() {
  ...
  useEffect(() => { queueMicrotask(() => void fetch("/api/v1/projects", { cache: "no-store" }).then(async (response) => { const body = await response.json() as { data?: ProjectSummary[]; error?: { message: string } }; if (!response.ok || !body.data) throw new Error(body.error?.message); setProjects(body.data); }).catch((reason) => setError(reason instanceof Error ? reason.message : "Không thể tải dự án.")).finally(() => setLoading(false))); }, []);
