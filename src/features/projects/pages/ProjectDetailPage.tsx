...
export async function ProjectDetailPage({ projectId, section }: { projectId: string; section: string }) {
  ...
  const schedule = section === "schedule" ? await getProjectSchedule(projectId, today.toISOString().slice(0, 10), end.toISOString().slice(0, 10)) : [];
  const overviewUpdates = section === "overview" ? (await listProjectUpdates(requireAuthenticatedUser(await getRequestUser()), projectId, { limit: 5 })).items : [];
  const healthHistory = section === "history" ? await listProjectHealthHistory(requireAuthenticatedUser(await getRequestUser()), projectId) : [];
