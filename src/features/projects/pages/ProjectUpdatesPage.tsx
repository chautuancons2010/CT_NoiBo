...
export async function ProjectUpdateNewPage({ projectId, initial }: { projectId: string; initial: { worksiteId?: string; content?: string; attendanceSessionId?: string } }) {
  ...
  return <div className="page-stack project-update-new"><BackLink href={`/projects/${projectId}/updates`} label="Cập nhật dự án" /><header><span>{project.code}</span><h1>Cập nhật dự án</h1></header><Card><ProjectUpdateForm initial={initial} projectId={projectId} worksites={project.worksites.filter((item) => item.status === "active")} /></Card></div>;
}
