...
export async function ProjectMonitoringPage({ recentOnly = false }: { recentOnly?: boolean }) {
  ...
  if (recentOnly) return <div className="page-stack"><header className="page-header"><div className="page-header__copy"><h2>Cập nhật gần đây</h2></div></header><div className="project-update-timeline">{summary.recentUpdates.map((update) => <ProjectUpdateCard key={update.id} showProject update={update} />)}</div></div>;
  return <div className="page-stack project-monitoring"><header className="page-header"><div className="page-header__copy"><h2>Theo dõi dự án</h2></div><Link className="button button--secondary button--md" href="/project-monitoring/issues">Trung tâm vấn đề</Link></header>
