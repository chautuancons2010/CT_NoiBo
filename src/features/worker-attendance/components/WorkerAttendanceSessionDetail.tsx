...
export function WorkerAttendanceSessionDetail({ sessionId }: { sessionId: string }) {
  ...
  return <div className="worker-detail">
    ...
    <div className="worker-wizard__title"><div><h2>{session.projectName}</h2><span>{session.worksiteName} · {new Date(`${session.date}T00:00:00`).toLocaleDateString("vi-VN")}</span></div><StatusBadge tone={session.status === "submitted" ? "success" : session.status === "needs_review" ? "warning" : "info"}>{labels[session.status]}</StatusBadge></div>
    ...
    <Card><div className="worker-detail-table"><div className="worker-detail-table__head"><span>Công nhân</span><span>Trạng thái</span><span></span></div>{session.entries.map((entry) => <div className="worker-detail-table__row" key={entry.id}><span><strong>{entry.workerName}</strong><small>{entry.employeeCode}</small></span><StatusBadge tone={entry.status === "present" ? "success" : entry.status === "unconfirmed" ? "neutral" : "warning"}>{labels[entry.status]}</StatusBadge><Button onClick={() => setEditingEntry(entry.id)} size="sm" variant="ghost">Điều chỉnh</Button></div>)}</div></Card>
