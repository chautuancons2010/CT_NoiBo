...
const [accountsResult, employeesResult, accountRolesResult, authUsersResult] = await Promise.all([
...
for (const [label, result] of [["app_accounts.rows", accountsResult], ["employees.rows", employeesResult], ["account_roles.rows", accountRolesResult]]) {
  if (result.error) {
    failed = true;
...
if (authUsersResult.error) {
  ...
  if (checks.account_employee_orphans || checks.accounts_without_role || checks.auth_without_account_link) failed = true;
}
